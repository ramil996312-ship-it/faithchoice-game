...
async function beginStoryPlay(key) {
  ...
  if (!content().STORIES?.[key]) {
    // Сеть оборвалась посреди догрузки текстов — не даём этой ошибке остаться необработанной
    // (обработчик клика не await'ит beginStoryPlay): выходим молча, loadScript() сам сбросил
    // свой кэш промиса при ошибке, так что повторный тап по карточке запустит закачку заново.
    try { await waitForStories(lang); }
    catch (e) { console.warn('[story] не удалось догрузить тексты истории, повторите попытку:', e); return; }
  }
  markStarted(key);
...
async function renderCurrent() {
  ...
  if (scene.choices && scene.choices.length) {
    ...
    // пройденной истории (см. её код), а не сбрасывает список наверх. Было 7с (до этого 1.6с —
    // не хватало времени даже заметить кнопку "Поделиться") — по просьбе пользователя сокращено
    // до 4с, этого уже достаточно, чтобы заметить/нажать "Поделиться", а марафон "слушать все
    // подряд" не простаивает лишние секунды между историями.
